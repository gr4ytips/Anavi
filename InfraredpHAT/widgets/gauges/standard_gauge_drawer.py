# widgets/gauges/standard_gauge_drawer.py
...
class StandardGaugeDrawer(BaseGaugeDrawer):
    def __init__(self, parent_widget):
        # Initialize the base class (BaseGaugeDrawer)
        super().__init__(parent_widget)
        logger.debug(f"StandardGaugeDrawer initialized for {parent_widget.objectName()}")

    def draw(self, painter, rect, sensor_name, current_value_animated, min_value, max_value, unit, gauge_style, colors):
        logger.debug(f"  Drawing Standard Gauge for {self.parent_widget.objectName()}. Value: {current_value_animated}, Style: {gauge_style}")
        painter.save()

        # The 'rect' parameter now represents the ENTIRE drawing area allocated for
        # both the sensor name and the gauge body.

        # Define height ratios for distributing space within the 'rect'
        sensor_name_area_ratio = 0.20 # Allocate 20% of 'rect' height for the sensor name
        ...
        # 2. Create a sub-rectangle for the main gauge body (including values and fill).
        ...
        # --- Draw the gauge frame and style within its allocated space ('gauge_body_drawing_rect') ---
        path = QPainterPath()
        path.addRoundedRect(QRectF(gauge_body_drawing_rect), 10, 10) # Gauge frame uses its new sub-rect
        ...
        if max_value > min_value:
            fill_height = int(gauge_body_drawing_rect.height() * ((current_value_animated - min_value) / (max_value - min_value)))
            fill_height = max(0, min(gauge_body_drawing_rect.height(), fill_height))
            # Fill rect's Y and height are relative to gauge_body_drawing_rect
            fill_rect = QRectF(gauge_body_drawing_rect.x(),
                               gauge_body_drawing_rect.y() + gauge_body_drawing_rect.height() - fill_height,
                               gauge_body_drawing_rect.width(), fill_height)
        ...
        self._draw_value_text(painter, gauge_body_drawing_rect, current_value_animated, unit,
                              ...
                              # Original: 0.15 * rect.height(). New: 0.15 / 0.8 * gauge_body_drawing_rect.height() = ~0.1875
                              QFont(self._get_themed_font_family('font_family', 'Inter'),
                                    int(gauge_body_drawing_rect.height() * 0.0875)))
        painter.restore()
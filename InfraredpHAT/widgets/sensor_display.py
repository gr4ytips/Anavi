...
class SensorDisplayWidget(QGroupBox):
    ...
    def __init__(self, title, unit, thresholds, metric_type, sensor_category, main_window, theme_colors, initial_gauge_type, initial_gauge_style, is_preview=False, parent=None):
        ...
        self.progressBar.setTextVisible(False) # IMPORTANT: Always hide native text; we draw custom text
    ...
    def _get_progress_bar_qss(self):
        ...
        # This font size is for the *native* QProgressBar text, which we are now hiding.
        # It's kept here for completeness but might not be strictly necessary for display.
    ...
    def update_value(self, value):
        ...
        if self.progressBar.isVisible():
            ...
            # CRITICAL FIX: Always hide native text for progress bars. Custom overlay handles text.
            self.progressBar.setTextVisible(False)
            logger.debug("  QProgressBar: Native text ALWAYS hidden for progress bars (custom drawing will be used).")
    ...
    def _update_ui_visibility(self):
        ...
        if self.current_gauge_type in ["Type 1 (Standard)", "Type 2 (Compact)", "Type 4 (Analog - Basic)", "Type 5 (Analog - Full)"]:
            ...
            # CRITICAL FIX: Always hide native text for progress bar, custom overlay draws the actual value
            self.progressBar.setTextVisible(False)
            logger.debug("  UI Visibility: Native QProgressBar text hidden (custom drawing will be used).")
    ...
    def paintEvent(self, event):
        ...
        # Corrected: Initialize analog_bg_color and analog_border_color first
        analog_bg_color = self._get_themed_color("analog_gauge_background", gauge_background_color.name())
        analog_border_color = self._get_themed_color("analog_gauge_border", gauge_border_color.name()) # Use string literal default here
        ...
        if self.current_gauge_type == "Type 1 (Standard)":
            ...
            # For Progress Bars, native QProgressBar handles fill. We draw custom threshold lines and text.
            is_horizontal = (self.current_gauge_type == "Type 6 (Progress Bar - Horizontal)")
            
            # Map the QProgressBar's *contentsRect* to the painter's coordinate system
            # This is crucial for drawing accurately within the visible progress bar area.
            progress_bar_content_rect = self.progressBar.contentsRect()
            mapped_progress_bar_content_rect = QRect(
                self.progressBar.x() + progress_bar_content_rect.x(),
                self.progressBar.y() + progress_bar_content_rect.y(),
                progress_bar_content_rect.width(),
                progress_bar_content_rect.height()
            )
            
            self._draw_progress_bar_threshold_lines(painter, mapped_progress_bar_content_rect, is_horizontal, gauge_critical_color)
            
            # CRITICAL FIX: Always draw custom text for progress bars to show actual value
            self._draw_progress_bar_text_overlay(painter, mapped_progress_bar_content_rect, Qt.Horizontal if is_horizontal else Qt.Vertical)
    ...
    def _draw_progress_bar_threshold_lines(self, painter, bar_rect, is_horizontal, line_color):
        ...
        :param bar_rect: The rectangle occupied by the QProgressBar's *contents*.
                         These coordinates are relative to the parent widget (SensorDisplayWidget).
        :param is_horizontal: Boolean, True if the progress bar is horizontal, False if vertical.
        ...
        logger.debug(f"  Bar Content Rect for drawing lines: {bar_rect.x()}, {bar_rect.y()}, {bar_rect.width()}, {bar_rect.height()}")
        painter.save()
        ...
        if is_horizontal:
            ...
            # Threshold lines should be vertical.
            ...
            if low_thr is not None:
                ...
                logger.debug(f"  _draw_progress_bar_threshold_lines: Drawn vertical low threshold line at ({x_pos_low:.1f}, {bar_rect.top():.1f}) to ({x_pos_low:.1f}, {bar_rect.bottom():.1f}) for {self.objectName()}. Value: {low_thr}.")
            ...
            if high_thr is not None:
                ...
                logger.debug(f"  _draw_progress_bar_threshold_lines: Drawn vertical high threshold line at ({x_pos_high:.1f}, {bar_rect.top():.1f}) to ({x_pos_high:.1f}, {bar_rect.bottom():.1f}) for {self.objectName()}. Value: {high_thr}.")
            ...
            # Threshold lines should be horizontal.
            
            if low_thr is not None:
                # Calculate the normalized position of the low threshold
                low_thr_norm_pos = value_to_normalized_pos(low_thr, self._min_value, self._max_value)
                # Map to Y-coordinate: top of bar + (1 - normalized_pos) * bar_height
                # (1 - normalized_pos) because 0.0 is bottom (high Y), 1.0 is top (low Y) visually.
                y_pos_low = bar_rect.top() + (1.0 - low_thr_norm_pos) * bar_rect.height()
                # Draw a horizontal line from left edge to right edge of the progress bar's content area
                painter.drawLine(bar_rect.left(), int(y_pos_low), bar_rect.right(), int(y_pos_low))
                logger.debug(f"  _draw_progress_bar_threshold_lines: Drawn horizontal low threshold line at ({bar_rect.left():.1f}, {y_pos_low:.1f}) to ({bar_rect.right():.1f}, {y_pos_low:.1f}) for {self.objectName()}. Value: {low_thr}.")
            ...
            if high_thr is not None:
                ...
                logger.debug(f"  _draw_progress_bar_threshold_lines: Drawn horizontal high threshold line at ({bar_rect.left():.1f}, {y_pos_high:.1f}) to ({bar_rect.right():.1f}, {y_pos_high:.1f}) for {self.objectName()}. Value: {high_thr}.")
    ...
    def _draw_progress_bar_text_overlay(self, painter, bar_rect, orientation):
        ...
        # For vertical bar, text length is constrained by bar's height after rotation.
        # For horizontal bar, text length is constrained by bar's width.
        ...
        text_height_at_current_font = temp_metrics.height() # Needed for vertical calculation

        if orientation == Qt.Vertical:
            # When text is rotated 90 deg, its 'width' (horizontalAdvance) maps to the bar's height.
            # Its 'height' maps to the bar's width.
            available_length_for_text = bar_rect.height() # The vertical length of the progress bar
            available_thickness_for_text = bar_rect.width() # The horizontal thickness of the progress bar
            ...
            if text_width_at_current_font > available_length_for_text * 0.9 or \
               text_height_at_current_font > available_thickness_for_text * 0.9:
                # Scale font based on the more restrictive dimension
                ...
                logger.debug(f"  Vertical Text font size adjusted for {formatted_value}. New size: {font_size}.")
        else: # Horizontal
        ...
        # Calculate where the text will be drawn in its *final* rotated/translated position
        # For clipping, we need paths in the *painter's local coordinates* after transformations.
        
        # Calculate the rectangle where the text will reside before transformation
        # This rect is relative to the current `painter`'s origin (which is `self` for paintEvent)
        ...
        painter.translate(bar_rect.center()) # Translate to the center of the progress bar content area
        if orientation == Qt.Vertical:
            painter.rotate(-90) # Rotate -90 degrees (counter-clockwise)
            # After rotation, the logical X axis is vertical (upwards) and Y axis is horizontal (leftwards).
            # The text_target_rect's width becomes height, height becomes width relative to screen.
            # We want the text to be centered at (0,0) after rotation for simplicity,
            # then draw its local rect.
            ...
            logger.debug(f"  Text Display Rect (after rotation prep): {text_local_rect_after_rotation}.")
        else: # Horizontal
            text_local_rect_after_rotation = QRectF(
                -text_bounds_width / 2,
                -text_bounds_height / 2,
            ...
            logger.debug(f"  Text Display Rect (horizontal, relative to center): {text_local_rect_after_rotation}.")
        ...
        # 3. Create a QPainterPath for the text itself (in the rotated/translated coordinate system)
        ...
        # The clips must be defined in the *current transformed coordinate system* of the painter.
        
        filled_clip_path = QPainterPath()
        unfilled_clip_path = QPainterPath()

        # Calculate the fill and unfilled rectangles *relative to the bar's current center* (0,0 in painter's translated system)
        # This is where the core logic for the split text comes in
        
        # Horizontal (original width, fill moves from left to right)
        if orientation == Qt.Horizontal:
            fill_x_start = bar_rect.left() - bar_rect.center().x() # Map to 0,0 center
            fill_width = bar_rect.width() * normalized_value
            
            filled_rect_clip = QRectF(fill_x_start, bar_rect.top() - bar_rect.center().y(), fill_width, bar_rect.height())
            filled_clip_path.addRect(filled_rect_clip) 
            
            unfilled_rect_clip = QRectF(fill_x_start + fill_width, bar_rect.top() - bar_rect.center().y(), bar_rect.width() - fill_width, bar_rect.height())
            unfilled_clip_path.addRect(unfilled_rect_clip)
        else: # Vertical (original height, fill moves from bottom to top, after rotation this becomes left to right)
            # After rotating -90, the original Y-axis is now horizontal (negative is right)
            # The original X-axis is now vertical (positive is up)
            # We are drawing text in this *rotated* space.
            
            # The "bottom" of the vertical bar (min value) is at original bar_rect.bottom().
            # After rotation by -90 about center, original (center_x, bottom_y) becomes (center_x + (bottom_y - center_y), center_y - (center_x - center_x))
            # simplified: (center_x + y_offset, center_y) where y_offset is distance from center.
            # This is complex. Let's simplify the clip paths in the *rotated* space.
            
            # In the rotated space, the bar_rect becomes conceptually "horizontal"
            # Its "length" is original bar_rect.height()
            # Its "thickness" is original bar_rect.width()
            
            # Fill goes from "left" (bottom of original vertical bar) to "right" (top of original vertical bar)
            # in the rotated coordinate system.
            
            rotated_bar_width = bar_rect.height() # The original height becomes the width in rotated view
            rotated_bar_height = bar_rect.width() # The original width becomes the height in rotated view

            # Normalized fill percentage (0 to 1) applied to the *rotated width*
            fill_width_rotated = rotated_bar_width * normalized_value
            
            # Origin of rotated rect is at (-rotated_bar_width/2, -rotated_bar_height/2) relative to current painter origin (which is bar_rect.center())
            
            filled_rect_clip = QRectF(
                -rotated_bar_width / 2, # Start from "left" of the rotated bar
                -rotated_bar_height / 2, # Top of the rotated bar
                fill_width_rotated,      # Width of the filled part
                rotated_bar_height       # Full height of the rotated bar
            )
            filled_clip_path.addRect(filled_rect_clip)

            unfilled_rect_clip = QRectF(
                -rotated_bar_width / 2 + fill_width_rotated, # Start from end of filled part
                -rotated_bar_height / 2,
                rotated_bar_width - fill_width_rotated,
                rotated_bar_height
            )
            unfilled_clip_path.addRect(unfilled_rect_clip)


        # Apply the clips and draw
        ...
        painter.setClipping(False) # Restore clipping to draw the second part
